...
def get_kyc_detail(kyc_id): ## user info using kyc id 
    try :
...
def get_user_info_using_uuid(user_uuid): ## user info using kyc id 
    try :
        user_detail = models.find_sql(
            logger=obj_admin.logger,
            table_name=ref_strings.Tables.user_master,
...
def approve_user_kyc(user_id, status):
    try:
        approve_status = models.update_sql(
            logger=obj_admin.logger,
            table_name=ref_strings.Tables.user_master,
            update_data={
                'kyc_status' : 1
...
def get_deleted_addresses(filters, order_dict, pagination):
    try:
        filters['status'] = 0
        deleted_addresses, total_count = admin_model.get_deleted_address(
            logger=obj_admin.logger,
            filters= filters,
            order_dict= order_dict, 
            pagination = pagination
        ...
        error = common_util.get_error_traceback(sys, e)
        obj_admin.logger.error_logger('get_deleted_addresses%s' %error)
        raise


def restore_deleted_addresses(address):
    try:

        address_exist = models.find_sql(
            logger=obj_admin.logger,
            table_name=ref_strings.Tables.address_list,
            filters={
                'address': address,
        ...
        if not address_exist:
            raise custom_exceptions.UserException(bitcoin_wallet_ref_string.Bitcoin.invalid_address)
        email = address_exist[0].get('email', False)
        if not email:
            raise custom_exceptions.UserException(ref_strings.Common.operation_failed)


        # TO REGISTER IN BLOCKCYPHER WALLET
        valid_wallet = models.find_sql(
            logger=obj_admin.logger,
            table_name=ref_strings.Tables.address_master,
            filters={
                'email': email
        ...
        if not valid_wallet:
            raise custom_exceptions.UserException(bitcoin_wallet_ref_string.Bitcoin.wallet_not_registered)

        wallet_name = valid_wallet[0].get('wallet_name')

        if not wallet_name:
            raise custom_exceptions.UserException(bitcoin_wallet_ref_string.Bitcoin.wallet_not_registered)

        blockcypher_helper.add_address_to_wallet(address, wallet_name)


        # updaing local db
        restore_status = models.update_sql(
            logger=obj_admin.logger,
            table_name=ref_strings.Tables.address_list,
            update_data={
                'status': 1
        ...
        btc_util.update_address_balance(address)
        ...
        error = common_util.get_error_traceback(sys, e)
        obj_admin.logger.error_logger('restore_deleted_addresses%s' % error)
        raise



#@added new plans on investment_plans tbl
def add_new_investment_plans(payload):
    try:
        obj_admin.create_logger()
        ...
        insert_data = {
            ...
            'minimum_investment': payload.get('minimum_investment'),
            'maximum_investment': payload.get('maximum_investment'),
            'profit': payload.get('profit'),
            'Instant_withdrawal': payload.get('Instant_withdrawal'),
            'capital_security': payload.get('capital_security'),
            'status' : 0
        }

        db_status = models.insert_sql(
            obj_admin.logger,
            ref_strings.Tables.investment_plans,
            insert_data
        )
        if not db_status:
            raise custom_exceptions.UserException(ref_strings.AdminCms.database_issue)
        ...
        return {
            'message': 'Investment Plans added Successfully '
        }
...
def get_investment_plans_list(filters, order_dict, pagination):
    try:

        investment_plan_list, total_count = admin_model.get_investment_plan_list(
            logger= obj_admin.logger,
            filters= filters, 
            order_dict=order_dict, 
            pagination=pagination
        ...
        for each_investment_plan_data in investment_plan_list:
            investment_plan_data = {
                ...
                'status' : each_investment_plan_data.get('status'),
        ...
        return_data = {
            ...
            'count' : len(investment_plan_return_list),
            'total_count' : total_count,
            'message' : ref_strings.Common.operation_sucess
        }
        ...
        error = common_util.get_error_traceback(sys, e)
        obj_admin.logger.error_logger('get_investment_plans_list%s' %error)
        raise



#@get investment plans details from  investment_plans table
def view_investment_plan_details(logger,uuid):
    try:
        investment_plan_list = admin_model.get_investment_plan_details(logger,uuid)
        if not investment_plan_list:
            return_data = []
            return {'data' : return_data}
        ...
        error = common_util.get_error_traceback(sys, e)
        obj_admin.logger.error_logger('view_investment_plan_details%s' %error)
        raise

#@ update and edit the invesyment plans details
def update_investment_plan(uuid, payload):
    try:
        update_condition = {
            'plan_name': payload.get('plan_name'),
            'minimum_investment': payload.get('minimum_investment'),
...
#delete investment plan from investment_plans table
def delete_investment_plan(uuid):
    try: 
        check_investment_plan =  models.find_sql(
            logger=obj_admin.logger,
            table_name=ref_strings.Tables.investment_plans,
            filters={
                'uuid' : uuid,
        ...
        if not check_investment_plan:
            raise custom_exceptions.UserException(ref_strings.Common.investment_plan_not_found)
        ...
        error = common_util.get_error_traceback(sys, e)
        obj_admin.logger.error_logger('delete_investment_plan%s' %error)
        raise

def check_investment_plan_exist_or_not(plan_name):
    try: 
        check_investment_plan =  models.find_sql(
            logger=obj_admin.logger,
            table_name=ref_strings.Tables.investment_plans,
        ...
        if check_investment_plan:
            raise custom_exceptions.UserException(ref_strings.Common.investment_plan_exist)
        ...
        error = common_util.get_error_traceback(sys, e)
        obj_admin.logger.error_logger('check_investment_plan_exist_or_not%s' %error)
        raise